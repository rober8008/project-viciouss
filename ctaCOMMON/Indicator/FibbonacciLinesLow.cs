...
{
    public class FibbonacciLinesLow : Shape_Indicator
    {
        public FibbonacciLinesLow() { this.In_Main_Chart = true; }
        public FibbonacciLinesLow(List<Candel> Data_Source, DateTime start_date, DateTime end_date, double start_value, double end_value, string color, string name): base(Data_Source, start_date, end_date, start_value, end_value, color, name)
        {
        ...
        public override void ApplyFormula()
        {
            ...
            Line rect0618 = new Line(quotesRange, this.Start_Date, this.End_Date, (minimun + (0.618 * C)), (minimun + (0.618 * C)), this.Color, this.Name + "(0.618)",SerieType.dashed);
            Line rect05 = new Line(quotesRange, this.Start_Date, this.End_Date, (minimun + (0.5 * C)), (minimun + (0.5 * C)), this.Color, this.Name + "(0.5)", SerieType.dashed);
            Line rect0382 = new Line(quotesRange, this.Start_Date, this.End_Date, (minimun + (0.382 * C)), (minimun + (0.382 * C)), this.Color, this.Name + "(0.382)", SerieType.dashed);
